...
EXCLUDE_KEYWORDS = [
    ...
    '국채', '채권', '단기', '머니마켓', 'MMF',
    # [fix20] 현금성(단기금리) ETF 차단 — 위 '채권/단기/머니마켓/MMF'와 동일 취지인데
    # CD금리·KOFR 계열만 키워드가 없어 유니버스로 새어들고 있었음(상위100 중 7종).
    # 하락장에서 이들만 모멘텀이 플러스라 top10에 진입해 주식 모멘텀 전략을 잠식함
    'CD금리', 'CD1년', 'KOFR',
    '달러', '엔화', '유로', '위안',
    '선물H', '(H)',
    ...
    '아시아', '신흥국', '이머징', '필라델피아',
    '차이나', '중국',   # [fix20] 누락 보완 — 중국 반도체 ETF 2종이 통과하고 있었음
]
...
#
# [fix20] 재현율 보강 — 상위100 유니버스 중 25종목이 "기타"로 새고 있었음.
#   같은 테마가 서로 다른 버킷으로 갈라지면 "섹터당 1개" 필터가 무력해진다.
#   실제 사고: 2026-07-20 매수 10종목에 HANARO Fn K-반도체(반도체)와
#   UNICORN SK하이닉스밸류체인액티브(기타)가 동시 편입 — 둘 다 사실상 반도체.
#   신규 섹터 5개(로봇/신재생/원자력/전력인프라) + 기존 섹터 키워드 확장으로 해소.
#   ⚠️ 순서 규칙: 구체적 테마가 포괄적 키워드보다 반드시 앞에 와야 한다.
#      - "로봇_자동화" > "자동차_모빌리티"  ("현대차로보틱스"는 로봇)
#      - "신재생_에너지" > "에너지_화학"    ("신재생에너지"가 에너지로 새면 태양광과 분리됨)
#      - "전력_인프라"/"통신_미디어" > "건설_부동산"  ("인프라"가 전부 건설로 빨려가던 문제)
SECTOR_KEYWORDS = [
    # ── 국내 섹터 (구체적인 것 먼저) ───────────────────────
    # fix7: "글로벌반도체" 제거 → 아래 별도 처리
    # fix20: 밸류체인형 반도체 ETF(엔비디아/하이닉스 협력사)가 기타로 새던 문제 보완
    ("반도체",          ["반도체", "AI반도체", "엔비디아", "하이닉스", "HBM"]),
    ("2차전지",         ["2차전지", "배터리", "리튬"]),
    ("바이오_헬스케어", ["헬스케어", "바이오", "제약"]),
    ("로봇_자동화",     ["로봇", "로보틱스", "휴머노이드"]),        # fix20 신규
    ("신재생_에너지",   ["신재생", "태양광", "풍력", "수소", "ESS"]),  # fix20 신규
    ("원자력",          ["원자력", "SMR"]),                        # fix20 신규
    ("전력_인프라",     ["전력", "AI인프라"]),                      # fix20 신규
    ("통신_미디어",     ["통신", "미디어", "방송", "엔터", "네트워크"]),
    ("IT_소프트웨어",   ["IT", "소프트웨어", "인터넷", "테크"]),     # fix7: "AI" 제거 → AI반도체가 반도체로 가야 함
    # fix20: "현대차"/"테슬라" 추가 — 밸류체인형 ETF(협력사 바스켓)가 기타로 새던 문제
    ("자동차_모빌리티", ["자동차", "모빌리티", "전기차", "현대차", "테슬라"]),
    ("에너지_화학",     ["에너지", "화학", "석유", "정유"]),
    ("금융_은행",       ["은행", "금융", "보험", "증권"]),
    ...
    ("철강_소재",       ["철강", "소재", "금속", "화학소재"]),
    ("중공업_방산",     ["중공업", "방산", "조선", "기계"]),
    # fix7: "금" 제거 → "중공업" "금융" 등에서 오매칭 방지
    # fix20: 대신 "금현물"/"KRX금"만 명시 추가 (KRX금현물이 기타로 새고 있었음)
    ("원자재_금",       ["골드", "원자재", "농산물", "금현물", "KRX금"]),
    ...
    # fix20: "삼성그룹"/"지주회사"도 사실상 코스피 대형주 바스켓이라 이 버킷으로 통합
    ("국내_대형지수",   ["KOSPI200", "코스피200", "코스피", "KOSPI",
                         "코리아TOP", "TOP10", "Top10", "TOP5", "Top5",
                         "ESG", "사회책임", "삼성그룹", "지주회사"]),
    ("국내_중소형",     ["코스닥150", "코스닥", "중소형", "스몰캡"]),   # fix20: "코스닥" 단독 추가
    ("국내_가치",       ["밸류업", "가치", "배당"]),

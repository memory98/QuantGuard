...
1. **표**: 구간 | 포트폴리오 | KOSPI200 | 상대수익률 | **매수했으면** | **가드효과** (STEP 3 출력 그대로)
2. **누적**: 첫 실행 총자산 → 최신 총자산의 수익률과 벤치마크 누적 비교
3. **해석** (아래 규칙을 그대로 적용):
   - 상대수익률 **+** = 시장보다 잘함 (계좌가 손실이어도 시장보다 덜 잃었으면 잘한 것)
   - 상대수익률 **−** = 시장보다 못함
   - **매수했으면(if_invested)** = 그 구간에 top10을 균등매수했을 때 수익률(가드 없었을 반사실). BEAR 현금 주에 "샀으면 얼마였나".
   - **가드효과(guard_effect)** = 실제 − 매수했으면. **+면 가드가 손실을 아낌**(현금 대피가 옳았음), **−면 가드가 기회를 놓침**(반등을 못 탐/휘프소). BEAR 주마다 이 값으로 가드의 실제 값어치를 정량화한다.
   - 데이터 4~5주 미만이면 반드시 "추세 판단은 아직 이르다"를 명시
4. **특이 관찰**: BEAR 발동 여부, 매매 이상(매수/매도 0건 등), 시그널 검증(모멘텀 base) 이상 시 언급.
   - ⚠️ `force_bull=True`인 quant_signals 파일은 테스트 실행이 실아카이브를 덮어쓴 것(market_status 가짜). top10 종목 자체는 유효하나 보고 시 명시.
